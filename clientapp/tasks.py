...
try:
    ...
    def send_quote_email_task(quote_id, recipient_email, subject, context=None):
        ...
        try:
            ...
            # Using Mailgun SMTP (simpler than API, no complex setup needed)
            try:
                logger.info(f"📧 Attempting to send email via Mailgun SMTP {settings.EMAIL_HOST}:{settings.EMAIL_PORT}...")
                ...
                # Check if it's a connection error (likely missing credentials on Render)
                if 'Connection' in str(type(smtp_error).__name__) or 'socket' in str(smtp_error).lower():
                    error_msg = "Email service connection failed. Check MAILGUN_SMTP_LOGIN and MAILGUN_SMTP_PASSWORD are set in Render environment."
                    logger.error(f"⚠️  {error_msg}")
                elif 'authentication' in str(smtp_error).lower() or 'login' in str(smtp_error).lower():
                    error_msg = "Mailgun authentication failed. Check credentials are correct."
                    logger.error(f"⚠️  {error_msg}")
                else:
                    error_msg = str(smtp_error)
                
                # For synchronous execution, just log and return error
